...
.EXAMPLE
    .\scripts\sync-github-secrets.ps1 -Only SUPABASE_URL,SUPABASE_KEY,ELEVENLABS_API_KEY
...
$props = @{}
foreach ($line in [IO.File]::ReadAllLines((Resolve-Path -LiteralPath $PropertiesPath).Path)) {
    $trimmed = $line.Trim()
...
foreach ($name in $wanted) {
    ...
    # Write-Output em pipe acrescenta newline; gh a descarta, mas o workflow
    # tambem normaliza CR/LF, entao os dois lados estao cobertos.
